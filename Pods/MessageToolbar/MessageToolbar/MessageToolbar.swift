...
public class MessageToolbar: UIView {
    ...
    public var pickFielButtonImage: UIImage? {
        didSet {
            pickFileButton.setImage(pickFielButtonImage, for: .normal)
        }
    }
...
//    public weak var delegate1: (UIViewController & MessageToolbarDelegate)? {
//        didSet {
//            sendButton.addTarget(self, action: #selector(sendMessage), for: .touchUpInside)
//            pickFileButton.addTarget(imagePickerService, action: #selector(self.pickfiel), for: .touchUpInside)
//        }
//    }
    
    
    @objc func pickfiel() -> Void {
        print("mostafa")
    }
    ...
    public  var pickFileButton: UIButton = {
        let button = UIButton(frame: .zero)
        button.translatesAutoresizingMaskIntoConstraints = false
        button.setTitle("", for: .normal)
        button.setImage(MessageToolbar.loadImage(name: "photo_gray"), for: .normal)
    ...
    private lazy var buttonsStackView: UIStackView = {
        let stackView = UIStackView(arrangedSubviews: [pickImageButton,pickFileButton, recordVoiceButton, sendButton])
